...
为什么 Redis 这么快了却不直接使用它来当主数据库？因为内存成本高且 Redis 提供的数据持久化仍然有数据丢失的风险。   
...
#### 旁路缓存策略是怎么样的？读写步骤？
旁路缓存是平时使用较多的一个缓存读写模式，比较适合读请求比较多的场景。   
...
但是，两个又有很大的不同：读写穿透模式是同步更新缓存和数据库，而异步缓存写入模式则是只更新缓存，不直接更新数据库，而是改为异步批量的方式来更新数据库。
...
2. **文件写入(write)**：将 AOF 缓冲区的数据写入到 AOF 文件中。这一步需要调用 `write`，`write` 将数据写入到系统内核缓冲区之后直接返回（延迟写）。注意：此时并没有同步到磁盘中。
