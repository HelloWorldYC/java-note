...
Netty 主要采用了 Reactor 模型，且通过细化线程池管理来处理不同的I/O任务  
Netty 的线程模型可以分为三个部分：  
- Boss线程组：负责接收客户端的连接请求
- Worker线程组：负责处理I/O读写操作，其中每个线程都是一个NioEventLoop，负责管理与处理与一个或多个Channel相关的所有I/O操作，处理逻辑通常通过ChannelPipeline中的各个ChannelHandler来完成
- 业务线程组（可选）：负责处理具体的业务逻辑  


### Netty 线程模型的特点

- 多线程处理：通过 Boss 线程组和 Worker 线程组分工明确、协同工作，提高处理并发连接的能力
- 多路复用：使用 Java NIO 的 Selector 实现 I/O 多路复用，提升 I/O 处理效率
- 异步非阻塞：I/O 操作采用异步非阻塞模式，充分利用CPU资源，避免因 I/O 阻塞导致的性能瓶颈（零拷贝）
- 高扩展性：支持自定义业务线程组，适应不同业务需求的场景，进一步提升系统性能和响应速度


### 为什么不选择使用原生的NIO而选择使用 Netty？

原生 NIO 存在的问题：  
1. 原生 NIO 接口较多，能支持更精细化的调用，但是对于通常的使用而言过于复杂
2. 存在一些 Bug，比如 Selector 空轮询，可能会导致 CPU 100%

使用 Netty 的优势：  
1. 封装了 NIO 的复杂 API，使用更为简单
2. 提供了优化的多线程模型，可以更高效地处理 I/O 事件和任务调度，提升并发处理能力
3. 支持多种传输协议（http、dns、tcp、udp等等），并且有自带编码器，解决了 TCP 粘包和拆包的问题
4. 解决了原生 NIO 的 Selector 空轮询等问题，且准备内部的细节做了优化
5. 采用了零拷贝机制，避免了不必要的拷贝，提升了性能
